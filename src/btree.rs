...
impl<'a> Pair<'a> {
    ...
    fn from_bytes(bytes: &'a [u8]) -> Self {
        bincode::options().deseirialize(bytes).unwrap()
    }
}
...
impl BTree {
    pub fn create(bufmgr: &mut BufferPoolManager) -> Result<Self, Error> {
        ...
        let mut meta = meta::Meta::new(meta_buffer_page.borrow_mut()) as RefMut<[_]>;
    ...
    match node::Body::new(node.header.node_type, node.body) {
        node::Body::Leaf(mut leaf) => {
            let slot_id = match leaf.search_slot_id(key) {
                ...
                Erro(slot_id) => slot_id,
            };
            if leaf.insert(slot_id, key, value).is_some() {
                buffer.is_dirty.set(true);
                Ok(None)
            } else {
                let prev_leaf_page_id = leaf.prev_leaf_page_id();
                ...
                if let Some(prev_leaf_buffer) = prev_leaf_buffer {
                    ...
                    prev_leaf_buffer.is_dirty.seat(true);
                }
                leaf.seat_prev_page_id(Some(new_leaf_buffer.page_id));
        ...
        node::Body::Branch(mut branch) => {
            ...
            if let Some((over_flwo_key_from_child, overflow_child_page_id)) =
                self.insert_internal(bufmgr, child_node_buffer, key, value)?
            {
                    ...
                    .is_some()
                {
                    ...
                    let overglow_key = branch.split_insert(
                        &mut new_branch,
...
mod tests {
    ...
    fn test() {
        ...
        let btree = BTree::create_page(&mut bufmgr).unwrap();
        btree
